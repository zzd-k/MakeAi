  ...
  <div class="p-5">
    ...
    <div class="art-card p-5 mb-5">
      <ElForm :inline="true" :model="searchForm">
        <ElFormItem :label="$t('pages.appOrderSettings.name')">
          <ElInput
            v-model="searchForm.name"
            :placeholder="$t('pages.appOrderSettings.placeholder')"
            clearable
          />
        </ElFormItem>
        <ElFormItem>
          <ElButton type="primary" @click="handleSearch">{{
            $t('pages.appOrderSettings.search')
          }}</ElButton>
          <ElButton @click="handleReset">{{ $t('pages.appOrderSettings.reset') }}</ElButton>
        </ElFormItem>
      </ElForm>
    </div>
    ...
    <!-- 操作按钮 -->
    <div class="art-card p-5 mb-5">
      <ElButton type="primary" @click="handleAdd">{{ $t('pages.appOrderSettings.add') }}</ElButton>
      <ElButton @click="handleExport">{{ $t('pages.appOrderSettings.export') }}</ElButton>
    </div>
    ...
    <div class="art-card p-5">
      <ArtTable :data="displayData" style="width: 100%" :border="true" :stripe="true">
        <template #default>
          ...
          <ElTableColumn
            :label="$t('pages.appOrderSettings.orderName')"
            prop="name"
            min-width="150"
          >
            <template #default="scope">
          ...
          <ElTableColumn
            :label="$t('pages.appOrderSettings.icon')"
            prop="icon"
            width="100"
            align="center"
          >
          ...
          <ElTableColumn
            :label="$t('pages.appOrderSettings.pathResource')"
            prop="path"
            min-width="200"
          >
            <template #default="scope">
          ...
          <ElTableColumn
            :label="$t('pages.appOrderSettings.createTime')"
            prop="createTime"
            width="160"
            align="center"
          >
          ...
          <ElTableColumn
            :label="$t('pages.appOrderSettings.edit')"
            width="150"
            align="center"
            fixed="right"
          >
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'AppOrderSetting' })
  ...
  const tableData = reactive<AppOrderItem[]>([
    {
      id: 121,
      name: '验证会话',
  ...
  const handleEdit = (row: AppOrderItem) => {
    console.log('编辑', row)
  }

  const handleDelete = (row: AppOrderItem) => {
    console.log('删除', row)
  }
